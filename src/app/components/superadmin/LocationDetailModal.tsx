...
import { StatCard } from './SuperAdminShell';
...
export function LocationDetailModal({ isOpen, onClose, id }: { isOpen: boolean; onClose: () => void; id: number | null }) {
  ...
  if (!summary) {
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="Location details" size="xl">
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={loc.name} size="xl">
      <div className="space-y-6">
        <div>
          <h3 className="text-sm font-medium text-gray-500">{loc.region} · {loc.address}</h3>
        </div>
